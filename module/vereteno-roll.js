...
import { veretenoModifierDialog } from "./veretenoPrayerModifier.js";
 ...
 * @param  {} rollData contains all data necessary to make a roll in vereteno.
 */
export async function veretenoRoll(chatOptions, rollData) {
  let errorObj = { error: "YZEvereteno.ErrorsInvalidSkillRoll" };
  ...
  const result = evaluateveretenoRoll(rollData, roll);
 ...
 * @param  {} origRoll
 */
export async function veretenoPushRoll(chatMessage, origRollData, origRoll) {
    origRollData.pushed = true;
    for (let part of origRoll.dice) {
      for (let r of part.results) {
        if (r.result !== CONFIG.YZEvereteno.maxRoll) {
          let newDie = new Die(20);
          await newDie.roll(1);
    ...
    const result = evaluateveretenoRoll(origRollData, origRoll);
...
function isValidRoll(rollData, errorObj) {
  ...
  switch (rollData.rollType) {
    ...
    case "advanced": // advanced skills
      if (skill <= 0) {
        errorObj.error = "YZEvereteno.ErrorsInvalidAdvancedSkillRoll";
        return false;
      }
      return attribute + skill > 0;
    case "weapon":
  ...
  errorObj.error = "YZEvereteno.ErrorsInvalidSkillRoll";
  return false;
}
...
export function evaluateveretenoRoll(rollData, roll) {
  let successes = 0;
  let maxRoll = CONFIG.YZEvereteno.maxRoll;
  let successesRoll = CONFIG.YZEvereteno.successesRoll;
  let failureRoll = CONFIG.YZEvereteno.failureRoll;
  roll.dice.forEach((part) => {
    part.results.forEach((r) => {
      if (r.result >= successesRoll) {
        successes++;
        if (r.result === maxRoll) {
          successes++;
        }
      } else if (r.result === failureRoll) {
...
async function showChatMessage(chatMsgOptions, resultData, roll) {
  let tooltip = await renderTemplate(
    "systems/yzevereteno/templates/sidebar/dice-results.html",
  ...
  await msg.setFlag("yzevereteno", "results", chatData.results);
  return msg;
}
...
function getRollIconKey(rollData) {
  const icon = CONFIG.YZEvereteno.skillIcons[rollData.skillKey];
  return icon ? CONFIG.YZEvereteno.icons[icon] : "";
}
...
function getRollAttributeName(rollData) {
  return CONFIG.YZEvereteno.attributes[rollData.attributeKey];
}
...
function getRollSkillName(rollData) {
  return CONFIG.YZEvereteno.skills[rollData.skillKey];
}
...
function getRollBlastRadius(rollData) {
  return CONFIG.YZEvereteno.ranges[rollData.blastRadius];
}
...
function getRollRange(rollData) {
  return CONFIG.YZEvereteno.ranges[rollData.range];
}
...
export async function veretenoChatListeners(html) {
  html.on("click", ".dice-push", (ev) => {
    let button = $(ev.currentTarget),
      messageId = button.parents(".message").attr("data-message-id"),
      message = game.messages.get(messageId);
    let results = message.getFlag("yzevereteno", "results");
    console.log(message);
    let originalRoll = message.rolls[0]; // TODO: handle this in a safer manner.
    if (message.flags.data?.results.pushed) {
      let errorObj = { error: "YZEvereteno.ErrorsAlreadyPushed" };
      ui.notifications.error(new Error(game.i18n.localize(errorObj.error)));
      return;
    } else {
      new veretenoModifierDialog(message, results.rollData, originalRoll).render(true);
    }
  });
}
/**
...
function createAutomaticFireFormula(totalDice, numberOfIgnoredOnes) {
  let formula = `${totalDice}d20`;
  for (let i = 0; i <= numberOfIgnoredOnes; i++) {
    formula = formula + ", 1d20x>=1";
  }
  return `{${formula}}`;
}
